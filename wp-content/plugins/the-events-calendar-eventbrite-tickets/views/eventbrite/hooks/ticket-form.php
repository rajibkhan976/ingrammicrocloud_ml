...
if( !class_exists('Tribe_Events_EventBrite_Template')){
	class Tribe_Events_EventBrite_Template extends Tribe_Template_Factory {
		...
		public static function the_tickets( $post_id ){
			$post_id = get_the_ID();
			$eventID = Event_Tickets_PRO::instance()->getEventId( $post_id );
			$html = '';
			if( !empty( $eventID ) && Event_Tickets_PRO::instance()->isLive($post_id) && tribe_event_show_tickets( $post_id ) ) {
				$html = sprintf( '<div class="eventbrite-ticket-embed" style="width:100%%;text-align:left">
									<iframe id="eventbrite-tickets-%1$s" src="http://www.eventbrite.com/tickets-external?eid=%1$s&amp;ref=etckt&v=2" style="height:200px;width:100%%;overflow:auto;"></iframe>
