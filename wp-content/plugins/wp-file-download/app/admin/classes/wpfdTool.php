...
class wpfdTool {
    ...
    function wpfd_import_notice(){
		...
		. 'function importWpfdTaxonomy(doit,button){'.PHP_EOL
			...
			.'if(doit===true){'.PHP_EOL
			    .'jQuery("#wpfd_error").after("<div class=\'updated\'> <p><strong>'. __('Categories imported into taxanomies. Enjoy!!!','wpfd') .'</strong></p></div>");'.PHP_EOL                           
			.'}'.PHP_EOL
	    ...
	    . '</script>';
            echo '<div class="error" id="wpfd_error">'
                    . '<p>'
                    . __('You\'ve just installed new version WP File Download, You can import your categories into taxonomies','wpfd')
                        . '<a href="#" class="button button-primary" style="margin: 0 5px;" onclick="importWpfdTaxonomy(true,this);" id="wpfdImportBtn">'.__('Import categories now','wpfd').' <span class="spinner" style="display:none"></span></a> or <a href="#" onclick="importWpfdTaxonomy(false,this);" style="margin: 0 5px;" class="button">'.__('No thanks ','wpfd').' <span class="spinner" style="display:none"></span></a>'
                    . '</p>'
                . '</div>';
       } else {
      ...
      $option_import_taxo = get_option('_wpfd_import_notice_flag');
        ...
        if($_POST['doit']==='true'){
            ...
            foreach ($files as $file) {
                ...
                $filename = $wp_upload_dir['basedir'].'/com_dropfiles/'.$file->catid.'/'.$file->file ;
    ...
    static function createCategoryIfNoneExist() {
        ...
        if(count($cats)==0) { //if there isn't any categories then create one              
            require_once $app->getPath().DIRECTORY_SEPARATOR.$app->getType().DIRECTORY_SEPARATOR.'models'.DIRECTORY_SEPARATOR.'category.php';            
            $modelCat = new wpfdModelCategory();
            $cat_id = $modelCat->addCategory(__('New category','wpfd'));              
        }
    }
