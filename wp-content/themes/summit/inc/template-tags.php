...
function summit_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'summit' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'summit' ), esc_html__( '1 Comment', 'summit' ), esc_html__( '% Comments', 'summit' ) );
		echo '</span>';
	}
