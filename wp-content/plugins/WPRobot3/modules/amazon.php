		...
		foreach($pxml->Items->Item as $item) {	
			if($yy >= $cnum) {
				...
				if ($rr<$count_this && $skipit == 0) {
					...
					$content = str_replace("{ean}", $ean, $content);
					...
					$customfield["amazonprice"] = str_replace("$ ", "", $price);
					...
					if($options['wpr_aa_shortcode'] == "Yes") {
						$single["content"] = '[wpramazon asin="'.$asin.'" keyword="'.$noqkeyword.'"]';
					} else {
		...
		<table class="addt" width="100%" cellspacing="2" cellpadding="5" class="editform"> 
			...
			<tr valign="top"> 
				...
				<select name="wpr_aa_site" id="wpr_aa_site">
					...
					<option value="in" <?php if ($options['wpr_aa_site']=='in'){echo "selected";}?>>Amazon.in</option>
				</select>				
				</td> 
			</tr>	
