		...
		if ( $item['added'] > 0 || $item['removed'] > 0 || $item['changed'] > 0 ) {
			...
			if ( sizeof( $item['added_detail'] ) > 0 ) {
				...
				foreach ( $item['added_detail'] as $file => $details ) {
					echo '<li class="file_change_detail"><strong>' . __( 'File', 'it-l10n-ithemes-security-pro' ) . '</strong>: ' . $file . '<br /><strong>' . __( 'Date', 'it-l10n-ithemes-security-pro' ) . '</strong>: ' . date( ' Y-m-d g:i a', ( isset( $details['mod_date'] ) ? $details['mod_date'] : $details['d'] ) ) . '</li>';
				}
