...
final class ITSEC_Lockout {
	...
	public function self_protect_metabox() {
		...
		<p><?php _e( 'Security is a delicate item. It does not care who you are, if it sees that you are trying to do something strange it will lock you out. This can be troublesome on sites with existing errors, particularly missing assets such as images and others.', 'it-l10n-ithemes-security-pro' ); ?></p>
		<p><?php _e( 'Use the button below to temporarily white list your IP from lockouts for 24 hours. It will still notify you of the situation but it will not lock you out of your site allowing you a chance to fix the issue.', 'it-l10n-ithemes-security-pro' ); ?></p>
		<p><?php _e( 'Please note that if your IP address changes at any time during the period (such as you switch locations) you could still inadvertently lock yourself out.', 'it-l10n-ithemes-security-pro' ); ?></p>

		<?php $temp = get_site_option( 'itsec_temp_whitelist_ip' ); ?>

		<?php if ( $temp !== false ) { ?>
			<?php echo __( 'Your IP Address', 'it-l10n-ithemes-security-pro' ) . ', <strong>' . $temp['ip'] . '</strong>, ' . __( 'is whitelisted until', 'it-l10n-ithemes-security-pro' ) . ' <strong>' . date( 'Y-m-d H:i:s', $temp['exp'] ) . '</strong>.'; ?>
		<?php } else { ?>
			<p class="submit"><a href="#" class="itsec_temp_whitelist_ajax button-primary"><?php _e( 'Temporarily Whitelist my IP', 'it-l10n-ithemes-security-pro' ); ?></a></p>
		<?php } ?>
