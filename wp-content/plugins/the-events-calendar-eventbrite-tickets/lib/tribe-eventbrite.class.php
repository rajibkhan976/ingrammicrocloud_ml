...
if ( !class_exists( 'Event_Tickets_PRO' ) ) {
	...
	class Event_Tickets_PRO {
		...
		public static $pluginVersion = '3.8';
		protected $cache_expiration = 86400; // defaults to 24 hours, use $this->get_cache_expiration() to apply filters
		public $pluginDir;
		public $pluginPath;
		public $pluginUrl;
		public $pluginSlug;
		...
		public function get_cost( $cost, $postId, $withCurrencySymbol ) {
			...
			$cached_cost = get_post_meta( $postId, '_EventbriteCost', true );

			// if the cache isn't expired we'll use the value stored there
			$cache_expiration = $this->get_cache_expiration();
			if ( ! empty( $cached_cost ) && time() < $cached_cost['timestamp'] + $cache_expiration ) {
				$cost = $cached_cost['cost'];
			} else {
				$eb_cost = $this->get_eb_prices( $postId );
				if ( $eb_cost ) {
					$cost = $eb_cost;
					update_post_meta( $postId, '_EventbriteCost', array('timestamp' => time(), 'cost' => $cost) );
				} else {
